<?php
    $nameGuest = "Jopie";
?>
<html>
    <head>
        <link rel="stylesheet" type="text/css" href="oefening1.css">
        <script src="oefening1.js"></script>
    </head>
    <body>
        <table>
            <tr>
                <td onclick="inputPlayer(1)"><div id="vakje1"></div></td>
                <td onclick="inputPlayer(2)"><div id="vakje2"></div></td>
                <td onclick="inputPlayer(3)"><div id="vakje3"></div></td>
            </tr>
            <tr>
                <td onclick="inputPlayer(4)"><div id="vakje4"></div></td>
                <td onclick="inputPlayer(5)"><div id="vakje5"></div></td>
                <td onclick="inputPlayer(6)"><div id="vakje6"></div></td>
            </tr>
            <tr>
                <td onclick="inputPlayer(7)"><div id="vakje7"></div></td>
                <td onclick="inputPlayer(8)"><div id="vakje8"></div></td>
                <td onclick="inputPlayer(9)"><div id="vakje9"></div></td>
            </tr>
        </table>
        <form action="contact.php" action="GET">
            <input type="text" name="aantalBeurten" id="aantalBeurtenBijhouder">
            <br><br>Vul hier je naam in:<br>
            <input type="text" name="naamSpeler" id="NaamSpeler">
            <input type="submit">
        </form>
        
        <input type="button" onclick="directeVerwijzing()">
        
        
        
    </body>
</html>